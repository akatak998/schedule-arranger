    ...
    layout(
      ...
      html`
        <form method="post" action="/schedules" class="my-3">
          ...
          <button class="btn btn-primary" type="submit">予定を作る　<i class="bi bi-calendar-week"></i></button>
        </form>
      `,
    ),
  );
...
app.post(
  ...
  async (c) => {
    ...
    await prisma.availability.upsert({
      where: {
        availabilityCompositeId: { // availabilityIdをcandidateId_userIdに変更し、candidateId_userIdをavailabilityCompositeIdに変更
